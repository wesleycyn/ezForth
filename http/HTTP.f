...
cls	
		...
		s" GET /webforth/ButtonBox.F HTTP/1.1" http$ lplace			
		crlf$  count http$ +lplace
		s" Host: www.jiemang.com" http$ +lplace	
		crlf$  count http$ +lplace    
    
		s" Connection: Keep-Alive" http$ +lplace	
		crlf$  count http$ +lplace
    
		s" Cache-Control: max-age=0" http$ +lplace	
		crlf$  count http$ +lplace
    
		s" Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8" http$ +lplace	
		crlf$ count http$ +lplace
		
    s" User-Agent: Mozilla/5.0 (Windows NT 5.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.72 Safari/537.36" http$ +lplace	
		crlf$  count http$ +lplace
		s" Accept-Encoding: gzip,deflate,sdch" http$ +lplace	

		crlf$  count http$ +lplace
		s" Accept-Language: zh-TW,zh;q=0.8,en-US;q=0.6,en;q=0.4" http$ +lplace	
