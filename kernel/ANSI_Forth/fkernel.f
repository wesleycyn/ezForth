...
<!-- saved from url=(0059)http://www.figtaiwan.org/Win32Forth6145xx/Chinese/fkernel.f -->
<html><head><meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1"></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">\ $Id: fkernel.f,v 1.95 2013/07/20 23:58:22 georgeahubert Exp $
\ WIN32FORTH KERNEL
\ Andrew McKewan
\ March 1994
...
CODE I          ( -- u )        \ push u, the value of the current loop index
                push    ebx
                mov     ebx, 0 CELLS [ebp]
                add     ebx, 1 CELLS [ebp]
                next    c;

CODE I'         ( -- u )        \ push u, the value of the current loop limit
                push ebx
                mov ebx, 1 CELLS [ebp]
                add ebx, # 0x80000000
                next c;

CODE J          ( -- u )        \ push u, the value of the outer loop index
                push    ebx
                mov     ebx, 3 CELLS [ebp]
                add     ebx, 4 CELLS [ebp]
                next    c;

CODE J'         ( -- u )        \ push u, the value of the outer loop limit
                push ebx
                mov ebx, 4 CELLS [ebp]
                add ebx, # 0x80000000
                next c;

CODE K          ( -- u )        \ push u, value of the second outer loop index
                push    ebx
                mov     ebx, 6 CELLS [ebp]
                add     ebx, 7 CELLS [ebp]
                next    c;

CODE K'         ( -- u )        \ push u, the value of the second outer loop limit
                push ebx
                mov ebx, 7 CELLS [ebp]
                add ebx, # 0x80000000
                next c;
...
\ *P User Variables with a negative offset are designed for system use and so are not normally
...
-3 CELLS ( init offset so TCB is 0 ) ( order IS important )
  DUP | USER TASK-STATUS CELL+ ( 32 bits for miscellaneous purposes. The lower 24 are reserved for the system)
...
( *1 )   3 CELLS + CONSTANT USERMIN   ( absolute min user area )
 TCB TASK-STATUS - CONSTANT USEROFFS  ( user offset, 3 cells )

\ *P The following words are for altering the task-status bits so u should be a power of 2 representing
\ ** the relevant mask.

: Task-Set       ( u -- )
\ *G Set task-status bit to true.
                 Task-Status @ or Task-Status ! ;

: Task-Clear     ( u -- )
\ *G Clear task-status bit.
                 Invert Task-Status @ and Task-Status ! ;

: Task-Flip      ( u -- )
\ *G Toggle task-status bit.
                 Task-Status @ xor Task-Status ! ;

: Task-Status?   ( u -- f )
\ *G Test task-status bit.
                 Task-Status @ and 0 &lt;&gt; ;

1 | Constant Main-Task
2 Constant Assync-Task

: Main?          ( -- f )
\ *G Returns true if this is the main (i.e. console task).
...
80 value maxascii         \ to support xchar extension
...
\ xTR ( adr n table -- ) 2011/11/29 ²n­ô
CODE TR \ ebx &lt;== table
        pop edi \ edi &lt;== len
        pop ecx \ ecx &lt;== addr
        push edx \ save edx
...
@@1:    cmp edi, ecx \ goto @@4, if edi &gt;= ecx
        ...
        mov [ecx], dl \ [ecx] &lt;== dl \ ­ì¦r¥À´«¦¨¤j¼g
        jmp short @@3 \ goto @@3
@@2:    inc ecx \ ecx &lt;== exc + 1 \ ¸õ¹L¤¤¤å¦r
@@3:    inc ecx \ ecx &lt;== exc + 1
        jmp short @@1 \ back @@1
@@4:    xor edi, edi \ edi &lt;= 0
        pop edx \ restore edx
...
CODE PARSE-NAME ( "&lt;spaces&gt;name" -- c-addr u ) \ parse the input stream
...
: IMMEDIATE     ( -- )  \ mark the last header created as an immediate word
                LAST @ N&gt;BFA BFA_IMMEDIATE ( TOGGLE ) over c@ or swap c! ;

\ Deprecated words will be removed from Win32Forth some time in the
\ future. When a deprecated word is used a warning message will be shown.
...
: "winlibrary   ( adr len -- )
        ...
        if
          ...
          0 ,                                   \ place-holder for the library handle
          ", 0 c,                               \ counted string
        then
        ;
...
|: mHeapAlloc   ( n -- link-addr fl )              \ allocate n bytes, return link address
               malloc-hflag malloc-haddr @       \ flags, heapaddress
               call HeapAlloc dup 0= ;

|: mHeapParm    ( rel-addr -- rel-addr 0 abs-heap-addr ) \ set up parms
               dup                               \ abs rel
...
: mHeapSize    ( rel-addr -- n )                 \ size of abs-addr bytes
               mHeapParm
...
: allocate      ( u -- addr fl )                \ ansi version of malloc
                malloc-adjlen +
...
: malloc        ( u -- addr )                   \ allocate dynamic memory
...
: realloc       ( u addr -- addr' fl )
                ...
                if      nip link&gt;mem TRUE       \ if not in list
...
2 4096 *        CONSTANT USERSIZE   \ user area size for task variables
...
\ *G Create a new chain.
                create here 0 , ['] noop compile,
                in-sys? &gt;system if sys-chain-link else chain-link then
                link, , system&gt;
                ;
...
\ *G Remove the specified item from the chain.
        begin   ( cfa link^ )
                dup @
        while   ( cfa link^ )
               2dup @ CELL+ @  =
...
HERE ," SRC\KERNEL\FKERNEL.F" CONSTANT KERNFILE \ kernel file
...
: RESTORE-INPUT ( ... 7 -- flag )             \ restore input
                ...
                IF
                  ...
                  source-position 0 source-id reposition-file THROW_FILEREADFAIL ?THROW
                  TIB MAXSTRING SOURCE-ID READ-LINE THROW_FILEREADFAIL ?THROW drop (SOURCE) !
                THEN
                FALSE ;
...
\ Added {: [a b c] [| [d e f]] [-- comments] :} as per Forth200x
\
\ Improved factoring
\
...
LABEL MOVE-LOCALS                               \ entry point for CLASS locals
  ...
  a; tcode-here cell-                               \ point at the offset in lea
                jmp     eax                     \ and leap...
  a; tcode-here 16 3 * + swap tcode-!                   \ correct the lea calculation
                pop     15 cells [ebp] \ nop   \ 16th
                pop     14 cells [ebp] \ nop
                pop     13 cells [ebp] \ nop
                pop     12 cells [ebp] \ nop
                pop     11 cells [ebp] \ nop
...
16 CONSTANT #-LOCALS                            \ must match code above!!!!
...
\ modfied version of a word suggested by Robert Smith, to get a word from the
...
|: BLNEXTWORD   ( -- addr ln )  \ check for next word, it fails if closing } or :} is missing
...
|: PFIND        ( addr -- addr FALSE | cfa -1 | cfa 1 ) \ find possible local. NOTE this is case-sensitive.
...
\   *** Note that PARMFIND translates the buffer in-place to uppercase
...
: &amp;OF           ( "name" -- addr )   \ get the address of a value or local
                ...
                STATE @ IF
                  ...
                  DOVALUE = 0= abort" &amp;OF expects either a LOCAL or a VALUE!"
...
|: (TYPEMSG)    ( n addr len -- )
                ...
                WHILE DUP CELL+ @ LAST-ERROR =           \ if it matches
                  IF 2 CELLS+ ?TYPE NULLMSG MSG !        \ print the message and clear it
                    PTRNULL THEN                         \ set ptr to null to stop loop
                REPEAT
...
|: MAIN         ( -- )                                 \ MAIN start forth main entry point
                ...
                Main-Task Task-Set                     \ set to identify main task
...
@@2:            mov     -4092 [edx], # 0        \ probe page at [edx]
                ...
                sub     esp, # USEROFFS         \ subtract user offset
                ...
                mov     Task-Status [UP] , dword # 0 \ default Task-Status is all bits off
...
: ;             ( -- )
                ...
                ?CSP ANON @  0= IF REVEAL THEN ANON OFF
                PARMS
                IF      COMPILE UNNESTP
                ELSE    COMPILE UNNEST
